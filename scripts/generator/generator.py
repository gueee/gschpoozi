...
"""
    ...
    def _generate_printer_cfg(self) -> str:
        ...
        try:
            if "printer_data-" in str(self.output_dir):
                # Extract instance ID from path like /home/user/printer_data-vzbot1/config
                parent = self.output_dir.parent.name
                if parent.startswith("printer_data-"):
                    instance_name = parent.replace("printer_data-", "")
        except Exception:
            pass
        ...
        lines = [
            "#######################################",
            f"# Klipper Configuration: {instance_name}",
