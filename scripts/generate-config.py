...
def generate_chopper_tuning_cfg(wizard_state: Dict) -> Optional[str]:
    ...
    lines.append("description: Iterate through chopper parameters at target speeds")
    ...
    lines.append("    M118 Testing {tpfd_values|length * tbl_values|length * toff_values|length} combinations")
    lines.append("")
    lines.append("    {% for tpfd in tpfd_values %}")
    lines.append("        {% for tbl in tbl_values %}")
    lines.append("            {% for toff in toff_values %}")
    lines.append('                _CHOPPER_TEST_COMBO STEPPER={stepper} SPEEDS="{speeds}" TPFD={tpfd} TBL={tbl} TOFF={toff} HSTRT=5 HEND=3')
    lines.append("            {% endfor %}")
    lines.append("        {% endfor %}")
    lines.append("    {% endfor %}")
    lines.append("")
    lines.append("    M118 Parameter sweep complete")
...
def generate_macros_cfg(wizard_state: Dict) -> str:
    ...
    lines.append("    # Park Z logic:")
    lines.append("    # - Always Z-hop up from current position")
    lines.append("    # - Never move Z downward while parking")
    lines.append("    # - Respect park_z_max as an upper cap (also capped to z_max-5)")
    lines.append("    {% set target_z = current_z + cfg.park_z_hop %}")
    lines.append("    {% set cap_z = z_max - 5 %}")
    lines.append("    {% if cfg.park_z_max is defined and (cfg.park_z_max|float) > 0 %}")
    lines.append("        {% set cap_z = [cap_z, cfg.park_z_max]|min %}")
    lines.append("    {% endif %}")
    lines.append("    {% set park_z = [target_z, cap_z]|min %}")
    lines.append("    {% set park_z = [park_z, current_z]|max %}")
    lines.append("    ")
